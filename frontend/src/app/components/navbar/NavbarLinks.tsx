            ...
            {links[props.role].map((link, index) => (
                <NavbarItem key={index}>
                    {props.position === NavbarPosition.BOTTOM ? (
                        <Link
                            href={link.path}
                            className={`${pathname === link.path ? "text-primary-600" : ""} flex w-fit flex-col items-center px-3 text-medium text-sm font-semibold`}
                        >
                            {link.icon}
                            <span className="mt-1">
                        ...
                        <Link
                            href={link.path}
                            className="flex w-fit flex-col items-center px-4 text-medium text-sm font-semibold"
                        >
                            <span>{link.label}</span>
                            <div
                                className={`h-1 w-7 rounded-full ${pathname === link.path ? "bg-primary-600" : ""}`}
                            />
                        </Link>
                    )}
                </NavbarItem>
